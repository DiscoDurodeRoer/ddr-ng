...
export class DdrChipComponent implements FormValueControl<string[]> {
  ...
  constructor() {
    effect(() => {
      if (this.maxValues() > 0 && untracked(() => this.value().length) > 0) {
        this.value.update((value: any[]) => value.slice(0, this.maxValues()));
      }
    });
  }
  ...
  onInsertElement() {
    ...
    if (
      this.valueInput() &&
      !this.value().find((v: string) => v == this.valueInput()) &&
      (!maxValues || this.value().length < maxValues)
    ) {
  ...
  checkValue() {
    if (
      this.valueInput() &&
      this.separator() &&
      this.valueInput().includes(this.separator())
    ) {
      this.valueInput.update(
        (value: string) => value.split(this.separator())[0],
      );
  ...
  onDelete(value: string) {
    ...
    this.value.update((values: string[]) =>
